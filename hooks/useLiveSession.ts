...
export const useLiveSession = ({ apiKey, systemInstruction }: UseLiveSessionProps) => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      // --- 3. LE PING DE RÉVEIL (LE CRUCIAL) ---
      // On attend 500ms pour être sûr que tout est stable
      setTimeout(async () => {
          console.log("📨 [TEST] Tentative d'envoi du message 'Bonjour'...");
          try {
              await session.send({
                  parts: [{ text: "Bonjour Ada ! Dis-moi si tu m'entends." }],
                  endOfTurn: true
              });
              console.log("📨 [TEST] Message envoyé ! Si l'IA marche, elle DOIT répondre.");
          } catch (err) {
              console.error("❌ [TEST] Échec de l'envoi du message :", err);
          }
      }, 500);

      // --- 4. Activation du micro ---
